...
for nfish =  [8]%@ SET
    ...
    cond_codes =[2000 2002]; %for nfish = 6 (#210412)
    
    for reject_on = [1 0]   %@ SET
        ...
        %--------------------------------------
        %1. APPLY FUNCTION TO EACH TRIAL-CONTROL 
        %--------------------------------------
        ...
        for triali = makeRow(sel_trials)
            ...
            for rowi = 1:size(movie,1)
                for coli = 1:size(movie,2)
                    ...
                    frames.meanslope(rowi, coli,triali) = meanslope;
                end %COLI
            end %rowi
            clear control_code condi
        ...
        for condi = makeRow(cond_codes)
            trials_cond  = intersect(sel_trials, find(VSDI.condition(:,1)==condi));
            meanslope = frames.meanslope;
            frames.meanslope(:,:,j) = squeeze(mean(meanslope(:,:,trials_cond),3));
