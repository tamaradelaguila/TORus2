...
# SETTINGS
sourcefile ="corr03_MATCH_correlation_NormRange220615wmean_normonly.xls"


sinkfile =  '220615_NORMALIZEDonly'  #
infotitle= sinkfile
...
data<-rawdata %>% select(c(3,4,5,9,10,11))

names(data)[1]="mA" ;  names(data)[4]="ibi0";
names(data)[2]=substr(names(data)[2], 1, 3); 
names(data)[3]=substr(names(data)[3], 1, 3); 
names(data)[5]=paste('N', names(data)[2], sep = ""); # label for normalized data: reconstruct from roi names
names(data)[6]=paste('N', names(data)[3],sep=""); 
...
A2norm<-ggplot(data = data, aes(x = Ndm2, y = ibi0, colour= mAF)) +
  geom_point( size = 2) + #"firebrick"     show.legend = TRUE
  labs(title  =  tit, x  =  "dm2_norm") + #'se = TRUE' for plotting intervals # y~model
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  annotate(geom= "text", x= 0, y=100,hjust=0,vjust=0 ,label =  labelcor2 , color='red')
...
svg(paste('REGRESSIONcolor_dm2norm_', sinkfile,'.svg'), width = 5, height = 5)
A2norm
dev.off()
A2norm
...
A4norm<-ggplot(data = data, aes(x = Ndm4, y = ibi0, colour= mAF)) +
  geom_point( size = 2) + #"firebrick"     show.legend = TRUE
  labs(title  =  tit, x  =  "dm4_norm") + #'se = TRUE' for plotting intervals # y~model
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  annotate(geom= "text", x= 0, y=100,hjust=0,vjust=0 ,label =  labelcor4_N , color='red')
#

svg(paste('REGRESSIONcolor_dm4norm_', sinkfile,'.svg'), width = 5, height = 5)
A4norm
dev.off()
A4norm
