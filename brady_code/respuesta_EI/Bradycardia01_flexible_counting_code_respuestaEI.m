...
% The code takes list of matfiles from a csv file (param 'source.filelist'), gets the spike information from them and
% perform the computation of the bradycardic response - both by the beats counting-based method, or the %ibi method -  outputting an excel
...
% Before using the code, the SETTINGS have to be adjusted according to the
% experimental design.
...
% will remain the 'stim' (see note below).
...
source.in = '/home/tamara/Documents/MATLAB/VSDI/TORus/data/bradi/respuestaEI'; 
source.out = '/home/tamara/Documents/MATLAB/VSDI/TORus/data/bradi/respuestaEI'; 
source.filelist= 'INPUT_filelist.txt';

design.RO = 0; %1 = with RO, 0= no RO. Will influence in whether trying to find blanks or not
design.kind = 'single_stim'; %'single_stim', 'discrim'

% RASTER SETTINGS: 'none', 'match_ibis', 'match_beats', 'custom'
% .......................................................................
config.rasterplot = 'match_ibis';

% ONLY FOR config.rasterplot = 'custom':
w.raster.pre = [8]; %in seconds
w.raster.post = [8];

% ADVANCED SETTING...
% .......................................................................

w.stimdur = 0; % (s) only used for plotting in the raster the stim offset
w.usonset = 0; % (s) ONLY FOR CONDITIONING onset respect to the CS onset for conditioning paradigms - to flag the presence of us in trial
...
w.baseline = 0; % ms of baseline (S delay)
...
% WINDOWS FOR beats
% ------------------------------------------------------------------------
        w.beats.pre = 8;  % pre-stimulus or baseline time window (s)
        w.beats.post = 8;  % post-stimulus time window (s)
...
% ...........................................
w.ibi.pre_n= 5; % nº of spikes for the baseline. NEEDED for config.ibiproxy.pre= 'n'
w.ibi.post_n = [8]; % post-stimulus nº of spikes. NEEDED for config.ibiproxy.post= 'n'
...
config.ibiproxy.pre = 'n*'; % skips the last ibi and takes one extra before instead 
...(to avoid an alteration of the baseline if we have to include manually one extra spike before the noise)
...
% 'specific' - one ibi with certain position; it needs the definition of
% the parameter 'w.ibi.locat'
config.ibiproxy.post = 'n_all';
...
% 'specific'
config.ibiproxy.comp = 'none'; % The most logical to use in  'ibiproxy.post'= 'n' or 's' is 'mean', since the baseline is a mean
...
% ADJUST NOISE, ONSET AND DELAY UNITS TO SECONDS
% .......................................................................
w.preUS_noise = preUS_noise/1000 ; 
w.USout= USnoise_ms/1000; 
w.Sout = stim_noise_ms/1000;  
w.shutdelay = w.shutdelay/1000;
w.baseline = w.baseline/1000; 
...
switch config.ibiproxy.post
    ...
    case 'n_all'
        labelibi.post = ['post_n all=', num2str(w.ibi.post_n) ];
...
for fili = 1:size(ref.filelist,1)
    ...
    excelname = fullfile(source.out, ['EI_' file(1:end-4) '.xls']);
    ...
    clearvars -except ecg SENS fili source export ref w file nombre beats cs us fishref exp_code setopt stimlist rowi stim1 stim2 design config labelibi excelname design
...
% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for CSi =  1:length(stimlist)
        ...
        for ii= 1:length(stim)
            ...
            % ------------------
            plus = find(stim(ii) < us & us <stim(ii)+ w.usonset+1);%+1 is a safety window
            if sum(plus) > 0; flag_us =1;  else flag_us = 0; end
            clear plus
            ...
            switch config.ibiproxy.post
                ...
                case 'n'
                    ...
                    idx_post = find(beats > stim(ii),w.ibi.post_n, 'first');
                ...
                case 'n_all'
                    ...
                    idx_post = find(beats > stim(ii),8, 'first');
                    selibi = ibi(idx_post);
                        % ----------------------------
            ...
            % ..............................
                    ...
                    coli = 13;
                    for nibi = 1:length(selibi)
                    output{rowi,coli} = round(selibi(nibi),2); %percentage of brady from inst freq ( >0 means brady)
                    coli = coli+1;
                    end
                        
    
            rowi = rowi +1;
            clear idx_pre idx_post meanpre meanpost perc peakS
        end %ii
        ...
        % ----------------------------------------------------
        %         header = {'stim(s)' 'n' 'CS' 'us' 'incl' '' 'npre' 'npost' 'npost_adj' '%beats' '' 'ibipre' labelibi.comp '%ibi' '' 'peakS'};
        header = {'stim(s)' 'n' 'CS' 'us' 'incl' 'BVtrial' 'npre' 'npost' 'npost_adj' '%beats' '' 'ibipre' 'ibi0' 'ibi1' 'ibi2' 'ibi3' 'ibi4' 'ibi5' 'ibi6' 'ibi7'};

        % -------------------
        % PRINT RASTER PLOT
        % -------------------
        switch config.rasterplot
            case {'match_ibis' , 'match_beats', 'custom' }
                figure
                aH = axes;
                ...
                for n = 1:length(rasterito)
                    row = rasterito{n};
                    N = ones(size(row))*n;
                    scatter(row, N, 15, 'k' ,'marker', 'o', 'MarkerFaceColor', 'k')
                    hold on
...
% Update history
% 17/11/22 - add the 'config.ibiproxy.pre= n*' that excludes the last spike from the pre_n spikes (and adds one before instead) 
% 14/11/22 - fixed excel name so matlab can read it as input in another
% code
% 19/10/22 - fix bug that forced the raster mode into 'match_ibis'
% 13/09/22 - flag the presence of us cchecking whether there is us inside
% the stim + w.usonset window (instead of stim + stimdur)
