...
% waves (%F) from which the %Fbased measures are extracted
plotwaves = 1; 
savewaves = 1;  %average %F waves (en
...
params{8,1} = ['waves measures: %F*10 '];

params{10,1} = 'extracted:';
params{10,2} = date;
...
% from: /home/tamara/Documents/MATLAB/VSDI/TORus/plot_code/informes_code/04_figure_sketch/groupplot_measures/groupplot4_boxplot.mat'
...
%--------------------------------------------------------------------------------------
... LATENCY FROM WAVES:  SECOND LOOP THROUGH CONDITIONS AND PIXELS TO GET ONSET LATENCY
...
end %for condition END OF CONDITION LOOP
...
for condition =  makeRow(cond_lohi)
    ...
    % --------------------------------------------------------------------------
    % GET AVERAGE MOVIE and calculate map. Use timerange set.
    % --------------------------------------------------------------------------
...
for condition =  makeRow(cond_lohi)
    ...
    if getR
        ...
        for roi_i = makeRow(selroi)
            ...
            % MEASURES FROM '%F' WAVES
            % ---------------------------------
            roiwavepeak= percFpeak(roi_i,ci);
            roiwavewmean = percFwmean(roi_i,ci);
            ...
            longF{i,15} = round(roiwavepeak,3);%
            longF{i,16} = round(roiwavewmean,3);
...
labels = {'id' 'roi' 'roi n' 'cond' 'peak' 'wmean' 'slope' 'onsetlat' 'Zpeak' 'Zmean' 'Zslope' 'peaklat' 'pFwavepeak' 'pFwavewmean' 'hemis', 'roii'};
...
% Update history:
% 13/10/22 - Add measures from %F waves output
