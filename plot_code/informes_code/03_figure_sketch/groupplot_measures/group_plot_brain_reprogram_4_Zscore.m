...
analysisref = 'bilat_'; %extra info for the name
...
for reject_on = [1]  %@ SET
    ...
    for suji  =  [5:8] %1:size(groupplot,1)
                ...
                case 'normal'
                    ...
                    for xi = 1:size(movieave,1)
                        for yi = 1:size(movieave,2)
                            ...
                            temp = devo_peak2peak(pixelwave, VSDI.timebase(idxrange), feedf.window, [], feedf.method, 0, 0);
        ...
        %% SPATIAL Z-SCORE AMONG CONDITIONS
        dim = size(maps.peak);
