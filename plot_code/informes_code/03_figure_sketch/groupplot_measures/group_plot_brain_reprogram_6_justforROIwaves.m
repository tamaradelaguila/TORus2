...
savein = '/home/tamara/Documents/MATLAB/VSDI/TORus/plot/informes/03_figure_sketch/def_figs/' ;%@ SET
...
%         cond_lohi = groupplot{suji,3}; 
        cond_lohi = [401:403]; %CHOOSE ONLY 3
...
%                 allroi_waves(:,roii,ci) = roi_TSave(movieave,roimask);
                tempwave = roi_TSave_percF_roiwise(movieave,roimask, meanF0);
                tempwave = squeeze(tempwave);
                detrended = filter_bleach_butterhigh(tempwave, 0.1, 6);
% 
%                 % DEBUGG: TEST DETREND
%                 figure
%                 plot(tempwave); hold on
%                 plot(detrended); legend 'original' 'detrended 0.01'
%                 
%                 return
%                 
%                 allroi_waves(:,roii,ci) = detended;
                allroi_waves(:,roii,ci) = detrended; 
            end %for roi_i
