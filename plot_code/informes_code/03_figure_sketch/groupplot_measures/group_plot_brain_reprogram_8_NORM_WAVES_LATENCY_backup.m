...
...................
    ...
    wi = 1; % counter for peak storage to get maximum for normalization

% FIRST LOOP TO GET GAIN-NORMALIZATION FACTOR
wi = 1;
for suji  =  sel_subjects %1:size(groupplot,1) SELECT included fish+condition
        ...
        % --------------------------------------------------------------------------
        % GET PEAK OF THE WAVE FOR GAIN-NORMALIZATION
        % --------------------------------------------------------------------------
        % Refroi wave
        refwave = roi_TSave(movieave,gain_ref_roimask);
        %                 reflat_wave = movmean(reflat_wave ,5);
        refwave = lowpass(refwave,10,fs);
        temp_gain = devo_peak2peak(refwave, timebase_adj, feedf.window, [], feedf.method, 0, 0);
        gain_peakidx = temp_gain.peakidx(2);
        gainpeak(wi) = refwave(gain_peakidx);
        wi = wi+1; 
        clear refwave temp_gain 
        end 
end

[peakmax, maxidx] = max(gainpeak);

% store information of reference fish for gain normalization
params{7,1} = 'gain normaliz ref';
refsuj =  groupplot{sel_subjects(maxidx),2}; 
params{7,2} = num2str(refsuj);

clear refsuj

        %% SECOND LOOP TO GET AND STORE NORMALIZED WAVES 
    ...
    for suji  =  sel_subjects %1:size(groupplot,1) SELECT included fish+condition
        ...
        gainfactor = (peakmax/localpeak)*10;
