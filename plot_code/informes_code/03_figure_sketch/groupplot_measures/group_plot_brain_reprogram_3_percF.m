...
%----------------------------------------------------------------
    ...
    for suji  =  1:size(groupplot,1)
        ...
        if ploton
            ...
            legend()
            tname = [num2str(VSDI.ref) ':' ref_movie(2:end) '-' roikind '(' refcase ')' ];
        ...
        if plotbar_on
            ...
            sgtitle(['fish-' num2str(VSDI.ref) '(' sourcedata ')'])
            subplot(2,3,1)
    ...
    % EXPORT FOR R
    % -------------------------------------------
    excelname = fullfile(savein, ['long_format_forR_group_' refcase 'ALL.xls']);
