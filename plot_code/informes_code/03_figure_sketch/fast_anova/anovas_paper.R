...
rawdata <-read.csv("long_format_forR_wmean210521_400_datafilt512_nodm3.csv") 



tit= "210521: window 0-250ms mean"; # for plots
file= 'RESULTS 210521 mean in window anatomical roi ANOVA 2w AND POST-HOC nodm3.txt' # to sink the data in
...
data <- data[c(4,5,6) ]
# If we want a subset
data <- data[data$mA %in% c("0", "0.4", "0.8"), ] 
...
bxp <- ggboxplot(
  data, x = "mA", y = "value",
  color = "roi", 
  ggtheme = ggthemes::theme_fivethirtyeight(), # choosing a different theme. theme_fivethirtyeight
  palette = colormap,
) +
  ggtitle(tit)
bxp
