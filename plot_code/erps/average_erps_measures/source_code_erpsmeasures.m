...
%         slopemean = mean(diff(waveW));
        ...
        text(-300,0.2,['A=' num2str(round(output.peakminusbasel,2))],'FontSize',6) % peak-to-peak amplitude
        text(-300,0.15,['S_x=' num2str(round(output.slopemax*10,2))],'FontSize',6) %
    ...
    if reject_on
        sgtitle([num2str(VSDI.ref),'pp', fcode , 'average waves cond=', num2str(code), '(cl)'])
    else
        sgtitle([num2str(VSDI.ref),'pp', fcode , 'average waves cond=', num2str(code)])
    end
    
    
    % Save
    if reject_on
        name = strcat([num2str(VSDI.ref),'pp', fcode , 'average waves cond=', num2str(code),'(clean)']);
    else
        name = strcat([num2str(VSDI.ref),'pp', fcode , 'average waves cond=', num2str(code)])
    end
